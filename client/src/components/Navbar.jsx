...
const Navbar = () => {
    ...
    const isActive = (route) => pathname === route ? "font-bold" : "";
    useEffect(()=>{
  ...
  return (
    ...
    <div className="hidden sm:flex items-center gap-8">
        ...
        <NavLink to='/products' className={isActive('/products')}>All Products</NavLink>
        ...
        <div onClick={()=>navigate('/cart')} className="relative cursor-pointer">
           <img className="w-6 opacity-80" src={isActive("/cart")?assets.nav_cart_icon:assets.cart_icon} alt="cart" />
            <button className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
        </div>
        {!user ?
        <>
          <NavLink to='/seller' className='text-primary'>Seller</NavLink>
       ...
       :<div className='relative group'>
            ...
            <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow-md border border-gray-200 py-2.5 w-30 rounded-md  text-sm z-40'>
                <li className='p-1.5 pl-3 hover:bg-primary/10 cursor-pointer' onClick={()=>navigate('/my-wishlist')}>My Wishlist</li>
...
<div onClick={()=>navigate('/cart')} className="relative cursor-pointer">
    ...
    <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
        ...
        <button onClick={()=>{setOpen(false);
            ...
            Login
        </button>: <button onClick={handleLogout} className="cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-primary-dull transition text-white rounded-full text-sm">
           Logout
        </button>}
    </div>
  }
